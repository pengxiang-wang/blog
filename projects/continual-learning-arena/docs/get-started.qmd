---
title: Get Started
description: A quick guide to set up CLArena, run a continual learning experiment and check the results.
...
After running the command, a folder will be created under `outputs/example/`containing all the information and outputs from the experiment, where `example` is the experiment name defined in the config. The folder will be named after the execution time (multiple runs will be stored separately if the command is executed more than once). You can check the results in this folder at any point during the experiment.

The results can include:

- `acc.csv` and `loss_cls.csv`: the performance matrix and average performance. See [here](../../../posts/continual-learning-beginners-guide.qmd#CL-metrics)
...
You can configure the output files by adjusting the configs controlling them to suit your needs.
